...
app.get('/api/health', (req, res) => {
  ...
  res.json({ status: 'ok', db: states[dbStatus] || 'unknown' });
});
  ...
  for (let i = 1; i <= retries; i++) {
    try {
      ...
      if (i === retries) {
        ...
        console.error('👉 Make sure MongoDB is running locally OR set MONGO_URI in .env to a MongoDB Atlas connection string.');
        console.error('   Example: MONGO_URI=mongodb+srv://<user>:<pass>@cluster0.xxxxx.mongodb.net/croptocash');
        // Keep server running — API will return DB errors gracefully
      } else {
...
// Global unhandled rejection handler — prevents nodemon crash
